...
exports.run = async(client, message, args) => {
    if (!db.has(`supanel_${message.guild.id}`)) return message.channel.send("**Server Stats** Sistemi Zaten Kapalı!)")
...
db.delete(`rekoronlineS_${message.guild.id}`)
  db.delete(`sesliK_${message.guild.id}`)
    ...
    message.channel.send("Server Stats Sistemi Başarıyla Kapatıldı!")
